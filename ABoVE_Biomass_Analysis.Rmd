...
wkdir = "G:/.shortcut-targets-by-id/15NjTWbCiv4HHa2ebSX0gJjPfaH74mX3E/ABoVE"
setwd(wkdir)
...
AlaskaCanada = read_sf(paste(wkdir, "/OtherSpatialDatasets/CanadaAlaska_Boundaries.shp", sep=""))
...
```{r}
Duncanson2023 = rast(paste(wkdir, "Duncanson2023/boreal_agb_202302061675663506_2760.tif", sep="/"))
Duncanson2023 = Duncanson2023[[1]]
Duncanson2023 <- terra::project(Duncanson2023, WGS84 <- "+init=EPSG:4326")
Duncanson2023_mask = mask(Duncanson2023, AlaskaCanada)
plot(Duncanson2023_mask)
...
calculate_global_stats(Duncanson2023_GSLcrop)

ggplot(data = Duncanson2023_GSLcrop)+
  geom_spatraster(data = Duncanson2023_GSLcrop) +
...
# Duncanson et al. 2023: Alaska Fire Disturbance stats 
```{r}
# Prepare all the data files needed to cover the fire disturbance region
Duncanson2023_fire1 = rast(paste(wkdir, "Duncanson2023/boreal_agb_202302061675666285_3551.tif", sep="/"))
Duncanson2023_fire2 = rast(paste(wkdir, "Duncanson2023/boreal_agb_202302061675669847_3456.tif", sep="/"))
Duncanson2023_fire3 = rast(paste(wkdir, "Duncanson2023/boreal_agb_202302061675666684_3645.tif", sep="/"))

Duncanson2023_fire1 = Duncanson2023_fire1 [[1]]
Duncanson2023_fire2 = Duncanson2023_fire2 [[1]]
Duncanson2023_fire3 = Duncanson2023_fire3 [[1]]
...
Duncanson2023_fire3 <- terra::project(Duncanson2023_fire3, WGS84 <- "+init=EPSG:4326")

Duncanson2023_4326 = rast("D:/ABoVE_Biomass/Duncanson1_EPSG4326.tif") # Duncanson1 was re-projected using gdal
```
```{r}
# Calculate for 2019 fire
...
fire1_resampled <- resample(Duncanson2023_fire1crop, Duncanson2023_fire2crop, method = "bilinear")
Duncanson2023_firecrop <- merge(fire1_resampled, Duncanson2023_fire2crop)
# Then create a mask to get the specific area
Duncanson2023_fire2019 = mask(Duncanson2023_firecrop, Fire2019)
calculate_global_stats(Duncanson2023_fire2019)

ggplot(data = Duncanson2023_fire2019)+
  geom_spatraster(data = Duncanson2023_fire2019) +
  labs(title = "Fire Disturbance 2019: Duncanson et al. 2023") +
  theme_light()+
  scale_fill_viridis(na.value="white", 
                     limits=c(0,400), 
                     name = "Biomass \n(Mg of C \nper ha)\n") +
...
# Duncanson2023 et. al Harvest Regions
```{r}
# Look at the 2019 fire for Duncanson
Duncanson_harvestcrop = crop(Duncanson2023_4326, ext(Harvest2019))
Duncanson2023_harvest2019 = mask(Duncanson_harvestcrop, Harvest2019)
calculate_global_stats(Duncanson2023_harvest2019)

ggplot(data=Duncanson_harvestcrop)+
  geom_spatraster(data=Duncanson_harvestcrop) +
  geom_sf(data = Harvest2019, fill="transparent", color = "#dc28bc", linewidth = 1.25)+
...
```{r}
Guindon2023_4326 = rast(paste(wkdir, "Guindon2023/Guindon2023_4326.tif", sep="/"))
...
```{r}
Guindon2023_Harvest = crop(Guindon2023_4326, ext(Harvest2019))
...
```{r}
Matasci2018 <-rast(paste(wkdir, "Matasci2018/matasci_4326.tif", sep="/"))
```
...
Matasci2018_harvestcrop = crop(Matasci2018, ext(Harvest2014))
...
ggplot(data=Matasci2018_harvestcrop)+
  ...
  geom_sf(data = Harvest2014, fill="transparent", color = "#dc28bc", linewidth = 1.25)+
...
```{r}
SotoNavarro2020 <- rast(paste(wkdir, "Soto-Navarro2020/Soto2020_merge.tif", sep="/"))
...
SotoNavarro2020_firecrop = crop(SotoNavarro2020_crop, Fire2009_ext)
...
ggplot(data = SotoNavarro2020_firecrop) +
  ...
  geom_sf(data = Fire2009, fill = "transparent", color = "#dc28bc", linewidth = 1.25) + # Outline for fire area
...
SotoNavarro2020_harvestcrop = crop(SotoNavarro2020_crop, ext(Harvest2009))
...
ggplot(data=SotoNavarro2020_harvestcrop)+
  ...
  geom_sf(data = Harvest2009, fill="transparent", color = "#dc28bc", linewidth = 1.25)+
...
```{r}
SpawnGibbs2020 = rast(paste(wkdir, "SpawnGibbs2020/aboveground_biomass_carbon_2010.tif", sep="/"))
...
```{r}
SpawnGibbs2020_firecrop = crop(SpawnGibbs2020_crop, Fire2009_ext)
...
```{r}
SpawnGibbs2020_Harvest  = crop(SpawnGibbs2020_crop, ext(c(-123.80, -123.30, 54.14, 55.17)))
Harvest_ext= ext(c(-123.5, -123.2,54.4, 54.6))
...
SpawnGibbs2020_harvestcrop = crop(SpawnGibbs2020_crop, ext(Harvest2009))
...
```{r}
Wang2020 = rast(paste(wkdir, "Wang2020/Wang2020_31_merge4326.tif", sep="/"))
...
Wang2020_GSL = crop(Wang2020_Bh12v11_1, GSL_ext)
Wang2020_GSL = Wang2020_GSL*0.01
...
```{r}
Wang2020_firecrop = crop(Wang2020, Fire2013_ext)
...
ggplot() +
  ...
  geom_sf(data = Fire2013, fill = "transparent", color = "#dc28bc", linewidth = 1.25) + # Outline for fire area
...
```{r}
Wang2020_harvestcrop = crop(Wang2020, ext(Harvest2013))
...
ggplot(data=Wang2020_harvestcrop)+
  geom_spatraster(data=Wang2020_harvestcrop) +
    geom_sf(data = Harvest2013, fill="transparent", color = "#dc28bc", linewidth = 1.25)+
...
#Xu et al. 2021: Setup
```{r}
Xu2021 = stack(paste(wkdir, "Xu2021/test10a_cd_ab_pred_corr_2000_2019_v2.tif", sep="/"))
Xu2021_20=Xu2021[[20]]
crs((Xu2021_20))
...
```{r}
Xu2021_GSL = crop(Xu2021_20_4326, GSL_ext)
...
```{r}
Xu2021_firecrop = crop(Xu2021_20_4326, Fire2018_ext)
Xu2021_fire2018 = mask(Xu2021_firecrop, Fire2018)

ggplot(data = Xu2021_firecrop) +
  geom_spatraster(data = Xu2021_firecrop) +
  geom_sf(data = Fire2018, fill="transparent", color = "#dc28bc", linewidth = 1.25)+
...
```{r}
Xu2021_harvestcrop = crop(Xu2021_20_4326, ext(Harvest2018))
...
ggplot(data=Xu2021_harvestcrop)+
  geom_spatraster(data=Xu2021_harvestcrop) +
    geom_sf(data = Harvest2018, fill="transparent", color = "#dc28bc", linewidth = 1.25)+
